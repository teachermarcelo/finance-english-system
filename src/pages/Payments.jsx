...
export default function Payments() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingPayment) {
        ...
        setToast({ message: 'Mensalidade atualizada!', type: 'success' });
      } else {
        const { error } = await supabase.from('payments').insert([data]);
        if (error) throw error;
        setToast({ message: 'Mensalidade cadastrada!', type: 'success' });
      }
      setModalOpen(false);
  ...
  const handleDelete = async (id) => {
    ...
    if (error) {
      ...
      setToast({ message: 'Mensalidade excluída!', type: 'success' });
      fetchData();
    }
  };
  ...
  const filteredPayments = payments.filter(p => {
    const matchesSearch = p.students?.name?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button onClick={() => { setEditingPayment(null); setFormData({ student_id: '', amount: '', due_date: '', status: 'pending', payment_date: '', notes: '' }); setModalOpen(true); }}>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan="5" className="text-center py-8 text-gray-400">Nenhuma mensalidade encontrada</td></tr>
              ) : (
                filteredPayments.map(payment => {
                  const statusInfo = getStatusInfo(payment);
                  return (
                    <tr key={payment.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 font-medium text-gray-800">{payment.students?.name || '-'}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {payment.status !== 'paid' && (
                            <button onClick={() => markAsPaid(payment.id)} className="p-1.5 hover:bg-green-50 rounded-lg text-green-600" title="Marcar como pago">
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                            </button>
                          )}
                          <button onClick={() => handleEdit(payment)} className="p-1.5 hover:bg-blue-50 rounded-lg text-blue-600">
                            <Edit size={16} />
                          </button>
                          <button onClick={() => handleDelete(payment.id)} className="p-1.5 hover:bg-red-50 rounded-lg text-red-600">
                            <Trash size={16} />
                          </button>
                        </div>
                      </td>
