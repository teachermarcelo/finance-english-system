...
export default function Financial() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingEntry) {
        const { error } = await supabase.from('financial_entries').update(data).eq('id', editingEntry.id);
        if (error) throw error;
        setToast({ message: 'Registro atualizado!', type: 'success' });
      } else {
        const { error } = await supabase.from('financial_entries').insert([data]);
        if (error) throw error;
        setToast({ message: 'Registro adicionado!', type: 'success' });
      }
      setModalOpen(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl font-bold text-gray-800">Entradas e Saídas</h2>
        <Button onClick={() => { setEditingEntry(null); setFormData({ description: '', amount: '', type: 'income', date: new Date().toISOString().split('T')[0], category: '', notes: '' }); setModalOpen(true); }}>
              ...
              {loading ? (
                ...
                filteredEntries.map(entry => (
                  <tr key={entry.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-600 hidden sm:table-cell">{entry.category || '-'}</td>
                    <td className="px-4 py-3 text-gray-600 hidden md:table-cell">{new Date(entry.date).toLocaleDateString('pt-BR')}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => handleEdit(entry)} className="p-1.5 hover:bg-blue-50 rounded-lg text-blue-600">
                          <Edit size={16} />
                        </button>
                        <button onClick={() => handleDelete(entry.id)} className="p-1.5 hover:bg-red-50 rounded-lg text-red-600">
                          <Trash size={16} />
                        </button>
                      </div>
                    </td>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="input"
                placeholder="Ex: Material, Aluguel..."
              />
            </div>
          </div>
          <div>
